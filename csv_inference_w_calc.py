...
# population statistics included. tested and running 2026-07-11
...
] # List of channels to be used for training and inference. Check spelling and consistency across samples. Adjust if needed.
...
calcchannels = ['34-ECD', '117-PC5.5', '45-CO'] # List of channels for which to calculate mean intensities per population
...
# Save df_calc to CSV in the results folder
csv_outfile = os.path.join(save_path, f'infer_calc_{date_time_str}.csv')
df_calc.to_csv(csv_outfile, sep=";", decimal=",", index=False)
...
# Attach concatenated MLP predictions to df_calc and re-save CSV
try:
    df_calc['y_pred_mlp'] = y_pred_mlp_full
    df_calc.to_csv(csv_outfile, sep=";", decimal=",", index=False)
    print(f'updated df_calc with y_pred_mlp and saved to {csv_outfile}')
...
try:
    ...
    # Also compute mean values for selected `calcchannels` per sample and mlp population
    try:
        means = df_calc.groupby(['sample', 'y_pred_mlp'])[calcchannels].mean()
        means_reset = means.reset_index()
        # Build mapping of sample -> {colname: value}
        mean_map = {}
        mean_cols = set()
        for _, r in means_reset.iterrows():
            sample = r['sample']
            label = int(r['y_pred_mlp'])
            for ch in calcchannels:
                colname = f'mlp_{label}_{ch}_mean'
                mean_cols.add(colname)
                mean_map.setdefault(sample, {})[colname] = r[ch]

        # Add mean columns to df_calc_results (one row per sample)
        for colname in sorted(mean_cols):
            df_calc_results[colname] = df_calc_results['sample'].map(lambda s: mean_map.get(s, {}).get(colname, np.nan))
    except Exception as e:
        print(f'could not compute per-population means: {e}')
# Save summary CSV
